...
<template>
  <div>
    ...
    <div class="fixed top-0 right-0 bottom-0 overflow-hidden au flex flex-col items-center py-4 px-5">
      <TransitionGroup tag="div"> <VtransToastComp v-for="(item, index) in monthList" v-bind:key="item" v-bind:label="item" v-bind:index="index" v-on:closed="(index: number)=>close(index)" class="my-2" /></TransitionGroup>
    </div>
  </div>
</template>
